...
#Include %A_ScriptDir%\libraries\settings-validator.ahk
#Include %A_ScriptDir%\libraries\windows-runtime.ahk
#Include %A_ScriptDir%\libraries\settings-toggle-service.ahk
...
VdeSettingsValidator.Validate(bootstrapSettings, logger)
global runtime := VdeWindowsRuntime()
...
try {
	...
	global settingsToggleService := VdeSettingsToggleService(settings, logger)
	global router := VdeEventRouter(app, settings, core, tray, overlayTooltip, logger, runtime, settingsToggleService)
	registrar := VdeHotkeyRegistrar(app, settings, router, core, logger)
	tray.BindRouter(router)
	runtime.EnableDarkTrayMenus()
    runtime.ApplyHotkeyBurstTuning(settings)
